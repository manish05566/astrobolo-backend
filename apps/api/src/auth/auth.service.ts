...
export class AuthService {
  ...
  async loginWithOtp(data: LoginOTPDto, req: Request) {
    try {
      ...
      let mobileExists = await User.findOne({ where: { mobile: data.mobile } });
      ...
      if (mobileExists) {
        ...
        mobileExists = await User.create({
          mobile: data.mobile,
          is_login: true,
          is_active: true,
        });
  ...
  // async update(data: updateDto, req: Request) {
  //   try {
  ...
  //     } else {
  //       return AppResponse.success({
  //         message: AppResponse.messages.user.notFound,
  //       });
  //     }
  //   } catch (error) {
  //     return AppResponse.failed({
